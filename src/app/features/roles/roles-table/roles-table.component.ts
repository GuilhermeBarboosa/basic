...
export class RolesTableComponent implements OnInit {
  ...
  constructor(
    ...
    private telaObservableService:TelaObservableService
  ) {}
  ...
  async ngOnInit() {
    ...
    this.telasDefault = this.telaObservableService.telasAll;

    if (this.role == this.rolesDefault.ROLE_ADMIN) {
        this.permissions = this.telaObservableService.telaAdmin
    } else {
  ...
  initTable() {
    this.roleService.getAll().subscribe((data) => {
      ...
      roleResponse.forEach((element: Role) => {
        ...
        element.role = this.utilsService.formatterString(element.role);
